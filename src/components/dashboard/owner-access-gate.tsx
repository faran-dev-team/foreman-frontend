...
export function OwnerAccessGate({ children }: { children: React.ReactNode }) {
  const { loading, accountStatus, accountMessage, shopId, error, refresh } =
    useShop();
  ...
  if (error && accountStatus !== "pending" && accountStatus !== "rejected") {
    return (
      <div className="mx-auto max-w-lg rounded-2xl border border-slate-200 bg-slate-50 px-6 py-8 text-center">
        <h2 className="text-lg font-semibold text-slate-900">
          Cannot reach the Foreman API
        </h2>
        <p className="mt-2 text-sm text-slate-700">
          {error}. Start the backend on port 8000, then retry. Your existing
          owner account is unchanged.
        </p>
        <button
          type="button"
  ...
  if (accountStatus === "active" && shopId) {
    return <>{children}</>;
  }
